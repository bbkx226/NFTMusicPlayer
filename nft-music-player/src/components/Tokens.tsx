...
const Tokens: FC<TokensProps> = ({ contract }) => {
  ...
  const loadMyTokens = async () => {
    ...
    const myTokens = await Promise.all(results.map(async (i: { args: any; tokenId: any; }) => {
        const args = i.args;
        if (!args) {
            return;
        }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 py-5">
            {myTokens.map((item, idx) => (
              <div key={idx} className="overflow-hidden">
                <audio src={item.audio} key={idx} ref={el => {if (el) audioRefs.current[idx] = el}}></audio>
                <div className="card">
                  ...
                  <div className="px-6 pt-4 pb-2">
                    <button onClick={() => resellItem(item)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                      Resell
                    </button>
                    <input
                      onChange={(e) => {
