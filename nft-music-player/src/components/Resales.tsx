...
const Resales: FC<ResalesProps> = ({ contract, account, loading, web3Handler }) => {
  ...
  const loadMyResales = async () => {
    ...
    const soldItems = listedItems.filter(i => results.some(j => j.args && i && i.itemId.toString() === j.args.tokenId.toString()))
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 py-3">
              {listedItems.map((item, idx) => (
                <div key={idx} className="overflow-hidden rounded-lg shadow-lg">
                  <audio src={item.audio} ref={el => { if (el) audioRefs.current[idx] = el; }}></audio>
                  <div className="w-full h-64 bg-cover bg-center" style={{ backgroundImage: `url(${item.identicon})` }}></div>
                  <div className="p-4 bg-white">
                    <p className="text-lg font-bold">{item.name}</p>
                    <div className="mt-2">
                      <button className="w-full px-4 py-2 text-white bg-gray-800 rounded-md" onClick={() => {
